...
import { getOrderDetile, getPayInfo } from "@/api";
export default {
  name: "Pay",
  data() {
    return {
  ...
  methods: {
    async open() {
      const imgUrl = await QRCode.toDataURL(this.payDetail.data.codeUrl);
      this.$alert(`<img src="${imgUrl}">`, "请使用微信扫码支付", {
        ...
        beforeClose: async (action, instance, done) => {
          ...
          if (action === "cancel") {
            this.$message({
              message: "请联系客服，电话：1313131313",
              type: "warning",
              showClose: true,
            });
            //点击支付成功
          } else if (action === "confirm") {
            const re = await getPayInfo(this.$route.query.data);
  ...
  async beforeMount() {
    //获取订单详情数据
    try {
      const re = await getOrderDetile(this.$route.query.data);
