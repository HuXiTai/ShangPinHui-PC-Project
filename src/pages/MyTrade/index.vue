<template>
  <div class="trade-container">
    ...
    <div class="content">
      ...
      >
        <span class="username" :class="{ selected: isChecked == index }">{{
          item.consignee
        }}</span>
        <p>
      ...
      <div class="detail">
        ...
        <ul
          class="list clearFix"
          v-for="item of tradeInfo.detailArrayList"
          :key="item.skuId"
        >
          <li>
    ...
    <div class="trade">
      ...
      <div class="receiveInfo">
        寄送至:
        <span>{{ (addressInfo[isChecked] || {}).fullAddress }}</span>
        收货人：<span>{{ (addressInfo[isChecked] || {}).consignee }}</span>
        <span>{{ (addressInfo[isChecked] || {}).phoneNum }}</span>
      </div>
    </div>
    <div class="sub clearFix">
      <a @click="placOrder" class="subBtn">提交订单</a>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...mapActions(["getTradeInfo", "getAddressInfo", "getOrderId"]),
    ...
    async placOrder() {
      ...
      try {
        const re = await this.getOrderId({ tradeNo, tradeInfo });
        this.$router.push({
          path: "/pay",
          query: { data: re },
        });
